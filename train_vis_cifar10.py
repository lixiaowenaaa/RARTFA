...
if __name__ == '__main__':
    ...
    parser.add_argument("--outf", type=str, default="logcifar10", help='path of log files')
    ...
    # Configure save root
    img_saveroot = './models/vis/cifar10/dc_img/'
    model_saveroot = './models/vis/cifar10/'
    ...
    train_dataset = torchvision.datasets.CIFAR10(root=img_root, train=True,
                                        download=True, transform=img_transform)                       
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size,
                                shuffle=True,
                                num_workers=0)
    tenval_dataset = torchvision.datasets.CIFAR10(root=img_root, train=False,
    ...
    for epoch in range(args.epochs):
        ...
        for i, (data, target) in enumerate(tbar):
            ...
            if i == 0: # archive images in order to save to logs
                images_disp.append(img_clean[0:12,:,:,:])
            # BPDA ATTACK 
            ...
            #    targeted=False)
            #adv_untargeted = bpda_adversary.perturb(img_clean, target)    
            ...
            output, c1_c, c2_c = modelC(img_clean)
            #output2, c1, c2 = modelC(adv_untargeted)

            loss1 = crossentropy(output, target)
            ...
            tbar.set_description('\r[%d/%d][%d/%d] [loss_cln: %.3f]' % \
                (epoch, args.epochs, i, len(train_loader), loss1.item()))
            #tbar.set_description('\r[%d/%d][%d/%d] [loss_cln: %.3f] [loss_adv: %.3f]' % \
            #    (epoch, args.epochs, i, len(train_loader), loss1.item(), loss2.item()))
        ...
        for i, (data, target) in enumerate(tbar):
            ...
            with torch.no_grad():
                ...
                output01, c11, c21 = modelC(images_disp[0])
            I_train = make_grid(images_disp[0], nrow=6, normalize=True, scale_each=True)
            writer.add_image('train/img', I_train, epoch)
            vis_fun = visualize_attn_softmax

            attn1,b1 = vis_fun(I_train, c11, up_factor=1, nrow=6)
            writer.add_image('train/attention_map_1', attn1, epoch)
            writer.add_image('train/attention_map_2', b1, epoch)
            
            attn2,b2 = vis_fun(I_train, c21, up_factor=2, nrow=6)
            writer.add_image('train/attention_map_3', attn2, epoch)
            writer.add_image('train/attention_map_4', b2, epoch)
